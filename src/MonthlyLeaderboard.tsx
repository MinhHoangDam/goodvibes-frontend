...
const MonthlyLeaderboard: React.FC<MonthlyLeaderboardProps> = ({ currentVibeDate }) => {
  ...
  const fetchLeaderboard = async (year: number, month: number): Promise<void> => {
    setLoading(true);
    try {
      const [sendersResponse, recipientsResponse] = await Promise.all([
        fetch(`${API_BASE_URL}/api/stats/monthly/top-senders?year=${year}&month=${month}&limit=5`),
        fetch(`${API_BASE_URL}/api/stats/monthly/top-recipients?year=${year}&month=${month}&limit=5`)
      ]);

      if (sendersResponse.ok && recipientsResponse.ok) {
  ...
  return (
    <div style={{
      ...
      {/* Month header */}
      <div style={{
        marginBottom: isCollapsed ? '0' : 'var(--hop-space-stack-lg)',
      ...
      {!isCollapsed && !loading && leaderboardData && (
        <>
          {/* Most Appreciated */}
          <div style={{ marginBottom: 'var(--hop-space-stack-xl)' }}>
            ...
            {leaderboardData.topRecipients.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--hop-space-stack-sm)' }}>
          ...
          <div>
            ...
            {leaderboardData.topSenders.length === 0 ? (
              <p style={{
                fontSize: 'var(--hop-body-sm-font-size)',
                color: 'var(--hop-neutral-text-weak)',
                fontStyle: 'italic',
